<template>
  <teleport to="body">
    <div>
      <div
        v-if="props.warningModalIsVisible"
        class="fixed top-0 left-0 h-screen w-full bg-gray-300/25 z-50"
      ></div>
      <transition @enter="focusYesBtn">
        <dialog
          v-if="props.warningModalIsVisible"
          open
          class="fixed z-50 top-[35vh] rounded-sm px-4 m-auto overflow-y-auto bg-gray-700"
          :class="[props.modalHeight, props.modalWidth]"
        >
          <div class="flex justify-center items-center my-4">
            ...
            <Icon
              ...
              class="h-20 w-20 text-white z-50"
            />
          </div>
          <div
            class="flex flex-col gap-y-4 justify-center items-center text-white"
          >
            <slot></slot>
            ...
            <div class="flex gap-x-10">
              <button
                ...
                @click="() => emits('emitYesBtn')"
                @keydown.enter.exact.stop.prevent="() => emits('emitYesBtn')"
                @keydown.space.exact.stop.prevent="() => emits('emitYesBtn')"
              >
                Yes
              </button>
              <button
                ...
                @click="() => emits('emitNoBtn')"
                @keydown.enter.exact.stop.prevent="() => emits('emitNoBtn')"
                @keydown.space.exact.stop.prevent="() => emits('emitNoBtn')"
